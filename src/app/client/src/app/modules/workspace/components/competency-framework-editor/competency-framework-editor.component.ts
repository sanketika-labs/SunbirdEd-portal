...
import { CompetencyFrameworkTreeComponent } from '../competency-framework-tree/competency-framework-tree.component';

export interface CompetencyFrameworkNode {
  id: string;
  name: string;
  code: string;
  description?: string;
  duration?: number; // Added duration property
  children: CompetencyFrameworkNode[];
}
...
export interface CompetencyFrameworkData {
  ...
  status: string;
  framework: string;
  rootNode: CompetencyFrameworkNode;
  createdBy: string;
  createdOn: string;
...
@Component({
  selector: 'app-competency-framework-editor',
  templateUrl: './competency-framework-editor.component.html',
  styleUrls: ['./competency-framework-editor.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class CompetencyFrameworkEditorComponent implements OnInit, OnDestroy {

  @ViewChild(CompetencyFrameworkTreeComponent) competencyFrameworkTreeComponent: CompetencyFrameworkTreeComponent;

  public unsubscribe$ = new Subject<void>();
  public contentId: string;
  public isLoading = true;
  public competencyFrameworkData: CompetencyFrameworkData;
  ...
  public durationFormControl = new FormControl('', [Validators.min(0)]); // Add duration FormControl
  public sectorFormControl = new FormControl('preschool');
  public profileFieldsFormControl = new FormControl([]);
  public node2TitleFormControl = new FormControl('', [Validators.required]);
  public node2OrderFormControl = new FormControl('', [Validators.required, Validators.min(1)]);
  public node2TimeLimitFormControl = new FormControl('no', [Validators.required]);
  public node2DurationFormControl = new FormControl('', [Validators.required, Validators.min(0)]);
  public node2LevelExamFormControl = new FormControl('no', [Validators.required]);
  public node2MinScoreFormControl = new FormControl('', [Validators.required]);
  public node2ObservableElementsFormControl = new FormControl([], [Validators.required]);
  ...
  private uniqueCodeValidator(control: FormControl): { [key: string]: any } | null {
    ...
    if (!this.competencyFrameworkTreeComponent) return null;

    // Get the current node's ID to exclude it from duplicate check
    // In FancyTree, use 'key' property for node ID
    const activeNode = this.competencyFrameworkTreeComponent.getActiveNode();
    ...
    const rootNode = this.competencyFrameworkTreeComponent.getRootNode();
  ...
  private getNodeIdByCode(code: string): string | null {
    ...
    return this.competencyFrameworkData ? findNodeById(this.competencyFrameworkData.rootNode, code) : null;
  }
  ...
  set nodeName(value: string) {
    ...
    if (this.selectedNodeData?.data?.metadata) {
      ...
      // Only update competencyFrameworkData.rootNode.name if this is the actual root node
      if (this.selectedNodeData.data.root === true) {
        if (this.competencyFrameworkData?.rootNode) {
          this.competencyFrameworkData.rootNode.name = value || this.resourceService?.frmelmnts?.lbl?.untitled || 'Untitled';
        }
      }

      // Update the tree node title dynamically
      if (this.competencyFrameworkTreeComponent) {
        this.competencyFrameworkTreeComponent.updateActiveNodeTitle(value || '');
      }
    }
  }
  ...
  get competencyFrameworkName(): string {
    if (this.competencyFrameworkData?.rootNode?.name) {
      return this.competencyFrameworkData.rootNode.name.trim() || this.resourceService?.frmelmnts?.lbl?.untitled || 'Untitled';
    }
    return this.resourceService?.frmelmnts?.lbl?.competencyFrameworkEditor || 'Skill Map Editor';
  }
  ...
  private getAllCodesFromTree(): string[] {
    ...
    if (!this.competencyFrameworkTreeComponent) return codes;
  ...
  ngOnInit(): void {
    ...
    this.activatedRoute.params.pipe(takeUntil(this.unsubscribe$)).subscribe(params => {
      ...
      this.activatedRoute.queryParams.pipe(takeUntil(this.unsubscribe$)).subscribe(queryParams => {
        if (queryParams['frameworkName']) {
          ...
          this.createNewCompetencyFrameworkWithFramework({
          ...
          this.createNewCompetencyFramework();
        } else {
          this.loadCompetencyFrameworkData();
        }
      });
    });
    ...
    this.nameFormControl.valueChanges.subscribe(value => {
      // Update the selected node's metadata name
      if (this.selectedNodeData?.data?.metadata) {
        this.selectedNodeData.data.metadata.name = value || '';
      }
      // Update the tree node title for any node (root or child)
      if (this.competencyFrameworkTreeComponent) {
        this.competencyFrameworkTreeComponent.updateActiveNodeTitle(value || '');
      }
    });
    ...
    this.codeFormControl.valueChanges.subscribe(value => {
      if (this.selectedNodeData?.data?.metadata) {
        ...
        if (this.selectedNodeData.data.root === true && this.competencyFrameworkData?.rootNode) {
          this.competencyFrameworkData.rootNode.code = value || '';
        }
    ...
    this.descriptionFormControl.valueChanges.subscribe(value => {
      if (this.selectedNodeData?.data?.metadata) {
        ...
        // Update the underlying skill map data for root node
        if (this.selectedNodeData.data.root === true && this.competencyFrameworkData?.rootNode) {
          this.competencyFrameworkData.rootNode.description = value || '';
        }
    ...
    // Duration form control sync for child nodes
    this.durationFormControl.valueChanges.subscribe(value => {
      if (this.selectedNodeData?.data?.metadata && this.nodeLevel > 0) {
        this.selectedNodeData.data.metadata.duration = value;
      }
    });
    this.sectorFormControl.valueChanges.subscribe(value => {
      if (this.selectedNodeData?.data?.metadata && this.nodeLevel === 0) {
        this.selectedNodeData.data.metadata.sector = value;
      }
    });
    this.profileFieldsFormControl.valueChanges.subscribe(value => {
      if (this.selectedNodeData?.data?.metadata && this.nodeLevel === 0) {
        this.selectedNodeData.data.metadata.profileFields = value;
      }
    });
    this.node2TitleFormControl.valueChanges.subscribe(value => {
      if (this.selectedNodeData?.data?.metadata && this.nodeLevel === 2) {
        this.selectedNodeData.data.metadata.name = value;
        // Update the tree node title live for child nodes
        if (this.competencyFrameworkTreeComponent) {
          this.competencyFrameworkTreeComponent.updateActiveNodeTitle(value || '');
        }
      }
    });
    this.node2OrderFormControl.valueChanges.subscribe(value => {
      if (this.selectedNodeData?.data?.metadata && this.nodeLevel === 2) {
        this.selectedNodeData.data.metadata.order = value;
      }
    });
    this.node2TimeLimitFormControl.valueChanges.subscribe(value => {
      if (this.selectedNodeData?.data?.metadata && this.nodeLevel === 2) {
        this.selectedNodeData.data.metadata.timeLimit = value;
      }
      if (value !== 'yes') {
        this.node2DurationFormControl.setValue('');
        this.node2DurationFormControl.clearValidators();
        this.node2DurationFormControl.updateValueAndValidity();
      } else {
        this.node2DurationFormControl.setValidators([Validators.required, Validators.min(0)]);
        this.node2DurationFormControl.updateValueAndValidity();
      }
    });
    this.node2DurationFormControl.valueChanges.subscribe(value => {
      if (this.selectedNodeData?.data?.metadata && this.nodeLevel === 2) {
        this.selectedNodeData.data.metadata.duration = value;
      }
    });
    this.node2LevelExamFormControl.valueChanges.subscribe(value => {
      if (this.selectedNodeData?.data?.metadata && this.nodeLevel === 2) {
        this.selectedNodeData.data.metadata.levelExamRequired = value;
      }
      if (value !== 'yes') {
        this.node2MinScoreFormControl.setValue('');
        this.node2MinScoreFormControl.clearValidators();
        this.node2MinScoreFormControl.updateValueAndValidity();
      } else {
        this.node2MinScoreFormControl.setValidators([Validators.required]);
        this.node2MinScoreFormControl.updateValueAndValidity();
      }
    });
    this.node2MinScoreFormControl.valueChanges.subscribe(value => {
      if (this.selectedNodeData?.data?.metadata && this.nodeLevel === 2) {
        this.selectedNodeData.data.metadata.minScorePercentage = value;
      }
    });
    this.node2ObservableElementsFormControl.valueChanges.subscribe(value => {
      if (this.selectedNodeData?.data?.metadata && this.nodeLevel === 2) {
        this.selectedNodeData.data.metadata.observableElements = value;
      }
    });
  }
  ...
  public onMetadataUpdate(event: any): void {
    ...
    if (event.action === 'save') {
      ...
      if (this.selectedNodeData?.data?.metadata) {
        ...
        // If this is the root node and name is being updated, update competencyFrameworkData.rootNode.name
        if (event.field === 'name' && this.selectedNodeData.data.root === true) {
          if (this.competencyFrameworkData?.rootNode) {
            this.competencyFrameworkData.rootNode.name = event.value || this.resourceService?.frmelmnts?.lbl?.untitled || 'Untitled';
          }
        }
        ...
        if (event.field === 'name') {
          ...
          // Update the tree node title dynamically
          if (this.competencyFrameworkTreeComponent) {
            this.competencyFrameworkTreeComponent.updateActiveNodeTitle(event.value || '');
          }
  ...
  private createNewCompetencyFramework(): void {
    // Create default structure compatible with FancyTree
    this.competencyFrameworkData = {
      id: 'new-skillmap',
      name: 'New Skill Map',
    ...
    this.extractAllCodes(this.competencyFrameworkData.rootNode);
    this.isLoading = false;
  }

  private createNewCompetencyFrameworkWithFramework(frameworkData: {name: string, code: string, description: string}): void {
  ...
  private loadCompetencyFrameworkData(): void {
    // Mock implementation - replace with actual service call
    this.competencyFrameworkData = this.generateMockCompetencyFrameworkData();
    this.extractAllCodes(this.competencyFrameworkData.rootNode);
    this.isLoading = false;
  }

  private generateMockCompetencyFrameworkData(): CompetencyFrameworkData {
    const rootNode: CompetencyFrameworkNode = {
      id: 'root',
      name: 'Preschool Competency Framework',
      description: 'Preschool Competency Framework',
      code: 'MATH_ROOT',
      children: [
        {
          id: 'beginner',
          ...
          code: 'MATH_ALGEBRA',
          duration: 60, // Example duration for child
          children: [
          ]
        },
        {
          ...
          code: 'MATH_GEOMETRY',
          duration: 45, // Example duration for child
          children: []
        }
      ]
    };
  ...
  private extractAllCodes(node: CompetencyFrameworkNode): void {
  ...
  public onTreeEvent(event: any): void {
    switch (event.type) {
      ...
      case 'nodeActivate':
        ...
        // Sync duration for child nodes
        if (this.nodeLevel > 0) {
          this.durationFormControl.setValue(this.selectedNodeData.data.metadata.duration || '', { emitEvent: false });
        } else {
          this.durationFormControl.setValue('', { emitEvent: false });
        }
        // Node level 2 fields
        if (this.nodeLevel === 2) {
          // Always use metadata.name for the title field
          this.node2TitleFormControl.setValue(this.selectedNodeData.data.metadata.name || '', { emitEvent: false });
          this.node2OrderFormControl.setValue(this.selectedNodeData.data.metadata.order || '', { emitEvent: false });
          this.node2TimeLimitFormControl.setValue(this.selectedNodeData.data.metadata.timeLimit || 'no', { emitEvent: false });
          this.node2DurationFormControl.setValue(this.selectedNodeData.data.metadata.duration || '', { emitEvent: false });
          this.node2LevelExamFormControl.setValue(this.selectedNodeData.data.metadata.levelExamRequired || 'no', { emitEvent: false });
          this.node2MinScoreFormControl.setValue(this.selectedNodeData.data.metadata.minScorePercentage || '', { emitEvent: false });
          this.node2ObservableElementsFormControl.setValue(this.selectedNodeData.data.metadata.observableElements || [], { emitEvent: false });
        }
        ...
        if (this.nodeLevel === 0) {
          this.sectorFormControl.setValue(this.selectedNodeData.data.metadata.sector || 'preschool', { emitEvent: false });
          this.profileFieldsFormControl.setValue(this.selectedNodeData.data.metadata.profileFields || [], { emitEvent: false });
        }
  ...
  private handleNodeAdded(event: any): void {
    this.updateCompetencyFrameworkFromTree();
  ...
  private updateCompetencyFrameworkFromTree(): void {
  ...
  public saveNodeChanges(): void {
    if (this.selectedNodeData && this.selectedNodeData.data && this.selectedNodeData.data.metadata) {
      ...
      if (this.validateNodeData(metadata)) {
        ...
        if (this.selectedNodeData.data.root === true) {
          if (this.competencyFrameworkData?.rootNode) {
            this.competencyFrameworkData.rootNode.name = metadata.name || 'Untitled';
          }
        }

        // Update the tree node title using the more reliable method
        if (this.competencyFrameworkTreeComponent) {
          this.competencyFrameworkTreeComponent.updateActiveNodeTitle(metadata.name || '');
        }
  ...
  public saveDraft(): void {
    ...
    setTimeout(() => {
      ...
      if (this.competencyFrameworkTreeComponent) {
        const validationResult = this.competencyFrameworkTreeComponent.validateAndHighlightNodes((nodeId: string) => {
          return this.validateNodeById(nodeId);
        });

        if (validationResult.hasErrors) {
          // Expand all tree nodes so user can see which nodes have validation errors
          this.competencyFrameworkTreeComponent.expandAllNodes();
          ...
          this.competencyFrameworkTreeComponent.clearAllHighlights();
          ...
          this.toasterService.success(this.resourceService?.frmelmnts?.lbl?.competencyFrameworkSavedDraft || 'Your skill map has been successfully saved as draft.');
        }
      }
    }, 100);
  }
  ...
  private syncFormToActiveNode(): void {
    if (this.selectedNodeData?.data?.metadata) {
      ...
      if (this.nodeLevel > 0) {
        this.selectedNodeData.data.metadata.duration = this.durationFormControl.value;
      }
      if (this.nodeLevel === 0) {
        this.selectedNodeData.data.metadata.sector = this.sectorFormControl.value;
        this.selectedNodeData.data.metadata.profileFields = this.profileFieldsFormControl.value;
      }
      ...
      if (activeNode && activeNode.data && activeNode.data.metadata) {
        ...
        if (this.nodeLevel > 0) {
          activeNode.data.metadata.duration = this.durationFormControl.value;
        }
        if (this.nodeLevel === 0) {
          activeNode.data.metadata.sector = this.sectorFormControl.value;
          activeNode.data.metadata.profileFields = this.profileFieldsFormControl.value;
        }
      }
    }
  }
  ...
  private triggerValidationForAllNodes(): void {
    if (!this.competencyFrameworkTreeComponent) return;

    try {
      const tree = this.competencyFrameworkTreeComponent.getTree();
      if (tree) {
  ...
  private validateNodeFormControls(node: any): void {
    ...
    if (isActiveNode) {
      ...
      if (!metadata.code || metadata.code.trim().length === 0) {
        ...
        const codeCount = this.countCodeOccurrences(this.competencyFrameworkTreeComponent.getRootNode().children[0], trimmedCode, nodeId);
  ...
  private validateNodeById(nodeId: string): boolean {
    if (!this.competencyFrameworkTreeComponent) return false;

    const node = this.competencyFrameworkTreeComponent.getNodeById(nodeId);
  ...
  public previewCompetencyFramework(): void {
    console.log('Preview functionality will be implemented');
  }

  public publishCompetencyFramework(): void {
  ...
  private updateLastModified(): void {
    this.competencyFrameworkData.lastModifiedOn = new Date().toISOString();
  }
  ...
  private getValidationErrorMessage(errorNodes: string[]): string {
    ...
    if (errorDetails.hasFormErrors) {
      ...
      // Generic fallback
      return this.resourceService?.frmelmnts?.lbl?.failedToSaveCompetencyFramework || 'Failed to save skill map, Please try again later.';
    }
  }
  ...
  // Method to get the current form output
  public getFormOutput(): any {
    const formOutput = {
      name: this.nameFormControl.value,
      code: this.codeFormControl.value,
      description: this.descriptionFormControl.value,
      enrolmentType: this.enrolmentTypeFormControl.value,
      timeLimit: this.timeLimitFormControl.value,
      duration: this.durationFormControl.value,
      sector: this.sectorFormControl.value,
      profileFields: this.profileFieldsFormControl.value,
      node2Title: this.node2TitleFormControl.value,
      node2Order: this.node2OrderFormControl.value,
      node2TimeLimit: this.node2TimeLimitFormControl.value,
      node2Duration: this.node2DurationFormControl.value,
      node2LevelExam: this.node2LevelExamFormControl.value,
      node2MinScore: this.node2MinScoreFormControl.value,
      node2ObservableElements: this.node2ObservableElementsFormControl.value
    };
