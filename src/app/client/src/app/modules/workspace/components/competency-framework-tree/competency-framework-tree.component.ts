...
@Component({
  selector: 'app-competency-framework-tree',
  templateUrl: './competency-framework-tree.component.html',
  styleUrls: ['./competency-framework-tree.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class CompetencyFrameworkTreeComponent implements OnInit, AfterViewInit, OnDestroy {
  
  @ViewChild('competencyFrameworkTree') public tree: ElementRef;
  @Input() public competencyFrameworkData: any;
  ...
  initialize() {
    if (this.competencyFrameworkData && this.competencyFrameworkData.rootNode) {
      this.buildRootNode();
    }
  }

  buildRootNode() {
    const data = this.competencyFrameworkData.rootNode;
  ...
  renderTree() {
    ...
    setTimeout(() => {
      ...
      if (rootFirstChild) {
        ...
        rootFirstChild.setExpanded(true);
        rootFirstChild.isExpandable = false;
        rootFirstChild.isExpanded = true;
        // Remove collapse/expand icon for root
        const $rootElement = $(rootFirstChild.li);
        $rootElement.addClass('skill-map-root-node');
        $rootElement.find('.fancytree-expander').hide();
      }
  ...
  updateNodeActions(node: any) {
    ...
    if (!node) {
      ...
      this.visibility.isRootNode = false;
    } else {
      ...
      // Only show add child for root node
      this.visibility.isRootNode = !!node.data.root;
    }
  ...
  private removePseudoElements(): void {
    setTimeout(() => {
      ...
      style.textContent = `
        .framework-tree .ui-fancytree::before,
        .framework-tree .ui-fancytree::after,
        .framework-tree .ui-fancytree ul::before,
        .framework-tree .ui-fancytree ul::after,
        .framework-tree .ui-fancytree li::before,
        .framework-tree .ui-fancytree li::after,
        .framework-tree .fancytree-container::before,
        .framework-tree .fancytree-container::after {
